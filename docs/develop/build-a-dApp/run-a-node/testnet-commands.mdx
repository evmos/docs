...
The `{{ $themeConfig.project.binary }} testnet` testnet subcommand makes it easy to initialize and start a simulated test network for testing purposes. 

In addition to the commands for [running a node](./run-a-node), the <ProjectValue keyword="binary" /> binary also includes a `testnet` command that allows you to start a simulated test network in-process or to initialize files for a simulated test network that runs in a separate process.
